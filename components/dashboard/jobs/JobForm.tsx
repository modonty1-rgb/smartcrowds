...
export function JobForm({ id, name, nameAr, description }: { id: string; name: string; nameAr?: string | null; description: string }) {
  const router = useRouter();
  const locale = useLocale();
  const isArabic = locale === 'ar';
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<z.input<typeof schema>>({ resolver: zodResolver(schema), defaultValues: { name, nameAr: nameAr || '', description } });
  ...
  return (
    <section className="space-y-4">
      <h1 className="text-2xl font-bold">{isArabic ? 'تعديل وظيفة' : 'Edit Job'}</h1>
